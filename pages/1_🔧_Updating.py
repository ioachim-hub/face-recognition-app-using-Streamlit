...
if choice == "Adding":
    ...
    if submit_btn:
        ...
        if name is None and image is None:
            ...
            st.warning(
                "Please check the image below to make sure "
                "you are deleting the right student"
            )
            st.image(image)
            del_btn = st.button(
                "Delete", key="del_btn", on_click=del_btn_callback, args=(id,)
            )
    ...
    def form_callback(old_name, old_id, old_image, old_idx):
        ...
        if new_image is not None:
            image = cv2.imdecode(
                np.frombuffer(new_image.read(), np.uint8), cv2.IMREAD_COLOR
            )
        ...
        if old_name is None and old_image is None:
            ...
            with st.form(key="my_form"):
                ...
                new_id = col1.text_input(
                    "ID", key="new_id", value=id, placeholder="Enter new id"
                )
                new_image = col1.file_uploader(
                    "Upload new image",
                    key="new_image",
                    type=["jpg", "png", "jpeg"],
                )
                ...
                st.form_submit_button(
                    ...
                    args=(old_name, id, old_image, old_idx),
                )
